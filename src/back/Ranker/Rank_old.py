...
class Rank:
    ...
    def final_results(self, user_query):
        logging.info(f"Start ranking")
        start_time = time.time()

        # ids = self._get_scores_sample(user_query)
        ids = self.rank_documents_proximity_multi_terms(user_query, 3)
        # ids = self._get_scores_cosine(user_query)
        logging.info(f"Number of results (_get_scores_sample): {len(ids)} ")
        logging.info(f"End ranking")
